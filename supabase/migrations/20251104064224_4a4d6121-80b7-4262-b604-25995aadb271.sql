-- Drop and recreate get_reports_admin function with incident_location
DROP FUNCTION IF EXISTS public.get_reports_admin(text, integer, integer);

CREATE FUNCTION public.get_reports_admin(p_status text DEFAULT NULL, p_limit integer DEFAULT 50, p_offset integer DEFAULT 0)
RETURNS TABLE(
  id uuid,
  reporter_id uuid,
  reporter_name text,
